...
Public Class Matern3d2 : Inherits CovarianceFunction
    ...
    Public Overrides Function k(Xi As Vector(Of Double), Xj As Vector(Of Double), sigmaSquaredF As Double, sigmaSquaredN As Double, lVector As Vector(Of Double)) As Double
        ...
        If Xi.Equals(Xj) Then
            Return sigmaSquaredF * (1 + Math.Sqrt(3) * Math.Sqrt(rSquared)) * Math.E ^ (-Math.Sqrt(3) * Math.Sqrt(rSquared)) + sigmaSquaredN
        Else
            Return sigmaSquaredF * (1 + Math.Sqrt(3) * Math.Sqrt(rSquared)) * Math.E ^ (-Math.Sqrt(3) * Math.Sqrt(rSquared))
        End If
    End Function
End Class
